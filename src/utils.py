...
def make_dates(experiment_dates: dict) -> pd.DataFrame:
    ...
    i = 0
    while True:
        train_start = pd.to_datetime(experiment_dates["train_start"]) + timedelta(
            weeks=4 * i
        )
        train_end = (
            pd.to_datetime(experiment_dates["test_start"])
            + timedelta(weeks=4 * i)
            - timedelta(weeks=1)
            - timedelta(weeks=4)
        )
        ...
        test_start = pd.to_datetime(experiment_dates["test_start"]) + timedelta(
            weeks=4 * i
        )
        test_end = (
            pd.to_datetime(experiment_dates["test_start"])
            + timedelta(weeks=(4 * i) + 4)
            - timedelta(weeks=1)
        )
...
def make_lag_features(
    data: pd.DataFrame, window_size: int, col_name: str, prefix_name: str, inplace=True
) -> pd.DataFrame:
    """
    ...
    for r in r_list:
        ...
        if google_trends:
            temp = (
                ...
                .mean()
                .reset_index()
                .rename(columns={"index": "level_1"})
            )
        else:
            temp = (
                df.groupby("product_category_name")[col_n]
                .rolling(r)
                .mean()
                .reset_index()
            )
        ...
        temp[f"{col_n}-rolling-{r}"] = temp[f"{col_n}-rolling-{r}"].apply(
